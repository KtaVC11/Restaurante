jQuery(function ($) {
	...
	**/
	...
	var cities = wcpcd_ajax.json;
	var selects = ['select.country_select', 'select.state_select', 'select.city_select'];
	...
	**/
	...
	function city_select($citybox, state) {
		var country = $citybox.closest('div').find('.country_to_state').val(); // $('#billing_country').val();
		...
		for (var index in current_cities) {
			if (current_cities.hasOwnProperty(index)) {
				if (current_cities[index]['city'] === city) {
					$container.find('input[id*="postcode"]').val(current_cities[index]['zip']);
				}
			}
		}
	}
	...
	**/
	$(document).on('click', 'a.edit_address', function(e) {
		// console.log('wcadmin_order_edit_billing_address_click');
